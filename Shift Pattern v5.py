...
def run_simulation(batchThreshold = 0.2, timeFrame = 100, totalCurve=totalCurve):
    ...
    startUpDelay_min = determineStartOrder(IM_pass_on_hourly_rate, AA_pass_on_hourly_rate, EO_batch) * 60

    IM_buffer = AA_buffer = VI_buffer = EO_buffer = EO_out = QU_buffer = 0
    EO_timer = totCost = 0
    demand = nominalDemand
    specialBatch = EO_batch
    startUpSpecial = startUpDelay_min
    ...
    for i in range(iterNum):
        ...
        if cycle_pos == 0:
            ...
            startUpSpecial = determineStartOrder(
                IM_pass_on_hourly_rate, AA_pass_on_hourly_rate, specialBatch
            ) * 60
        ...
        if not peakAlarm:
            if shift_of_day == 0:
                run_schedule = True
                IM_ttb = determineTTB(EO_batch, IM_pass_on_hourly_rate) * 60
                VI_ttb = determineTTB(EO_batch, VI_total_kept_hourly_rate) * 60
                startUp = startUpDelay_min
                VI_ends = VI_ttb + startUp

            if shift_of_day == 1:
                run_schedule = True
                IM_ttb = determineTTB(specialBatch, IM_pass_on_hourly_rate) * 60
                VI_ttb = determineTTB(specialBatch, VI_total_kept_hourly_rate) * 60
                startUp = startUpSpecial
                VI_ends = VI_ttb + startUp
        ...
        for grp in quarantine_timers:
            grp[0] = grp[0] - 1

        if cycle_pos < 450 and run_schedule:

            if cycle_pos < IM_ttb:
                IM_buffer += IM_pass_on_hourly_rate / 60
                totCost   += IM_scrap_hourly_rate / 60

            if startUp <= cycle_pos < IM_ttb:
                moved = min(AA_pass_on_hourly_rate / 60, IM_buffer)
                IM_buffer -= moved
                AA_buffer += moved

            if startUp <= cycle_pos < VI_ends:
                moved = min(VI_total_kept_hourly_rate / 60, AA_buffer)
                AA_buffer -= moved
                VI_buffer += moved
                totCost += VI_total_cost_hourly_rate / 60

            if cycle_pos >= VI_ends and EO_ready:
                EO_buffer += VI_buffer
                VI_buffer = 0
                AA_buffer = 0
                IM_buffer = 0
    ...
    on_time_percent = 100 * total_output / total_demand
...
plt.xlabel('Time [Hours]')
plt.ylabel('Cumulative Units Released')
plt.grid()

plt.figure()
plt.bar(day_times, daily_throughput, width=0.5)
plt.title('Daily Throughput')
plt.xlabel('Day')
plt.ylabel('Units Released Per Day #')
...
#%% 
# Sweep and Average Batch Threshold Values
